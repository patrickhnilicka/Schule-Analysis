          ...
          \begin{enumerate}
              ...
              \item 	Zeichen Sie die Graphen von $u$ als Funktion von $x$, von $y$ als Funktion von $u$ und von $y$ als Funktion von $x$ in die folgenden Koordinatensysteme (achten Sie auf die Achsenbeschriftung).
                    ...
                    \begin{minipage}[t]{0.25\textwidth}
                        \includegraphics[width=\textwidth]{im/KettenregelLA-1.png}
                    \end{minipage}
                    \begin{minipage}[t]{0.5\textwidth}
                        \includegraphics[width=\textwidth]{im/KettenregelLA-2.png}
                    \end{minipage}

                    \begin{center}
                        \includegraphics[width=0.36\textwidth]{im/KettenregelLA-3.png}
                    \end{center}
...
\begin{enumerate}
    \item Bestimmen Sie die Ableitung der folgenden Funktionen:
          ...
          \end{enumerate}
    \item Die Kettenregel kann auch auf die Verkettung von mehr als zwei Funktionen angewendet werden. Leiten Sie die folgenden Funktionen ab:
          $$y = \sqrt{(x^2+1)^3+x}$$
    \item Bestimmen Sie den Wert des Parameters \( a \in \mathbb{R} \) so, dass für die Funktion \( f \) gilt: \( f'(1) = 24 \).
          \begin{enumerate}
              \item \( f(x) = (2x - a)^3 \)
              \item \( f(x) = -\dfrac{6}{\sqrt{a x}} \) (Hinweis: Sie können die Funktion auf verschiedene Arten Ableiten: Direkt mit der Quotientenregel oder zuerst umformen zu $f(x) = -6 (ax)^{-\frac{1}{2}}$ )
                    %   \item \( f(x) = \dfrac{2a}{a - 4x} \)
          \end{enumerate}


    \item Bestimmen Sie die Gleichung der Tangente im Punkte \( P \) des Graphen der gegebenen Funktion \( f\colon x \mapsto y \).
          \begin{enumerate}
    ...
    \item Zeigen Sie: Die Gerade \( g\colon y = 2x \) ist Tangente der Kurve \( f\colon y = 4 \sqrt{2x - 4} \).
\end{enumerate}
...
\begin{enumerate}
    ...
    \item Lesen Sie die Funktion als Verkettung
    ...
    \item Bestimmen Sie den Wert des Parameters \( a \in \mathbb{R} \), so dass \( f'(1) = 24 \).
          \begin{enumerate}
    ...
    \item Bestimmen Sie die Gleichung der Tangente im Punkt \( P \).
          \begin{enumerate}
    ...
    \item Zeigen Sie: \( g(x) = 2x \) ist Tangente an \( f(x) = 4\sqrt{2x - 4} \)

          $$f'(x) = g'(x) \iff \dfrac{4}{\sqrt{2x - 4}} = 2 \iff x = 4$$

          Prüfen Sie ob $f(4) = g(4)$ ist:
...
\begin{enumerate}
    \item Unten ist der Graph der Sinusfunktion im Bereich $x \in [0, 2\pi]$ dargestellt. Skizzieren Sie in das gleiche Koordinatensystem den ungefähren Verlauf des Graphen der Ableitung $\frac{d}{dx} \sin(x)$, indem Sie
          ...
          \begin{center}
              \includegraphics[]{im/sinuskurve.png}
          \end{center}
\end{enumerate}
...
\begin{theo}{Die Ableitung von Sinus und Cosinus}{}
...
\begin{enumerate}
    \item Bestimmen Sie die Ableitung der folgenden Funktionen:
          \begin{enumerate}
              ...
              \item $y=\sqrt{\cos(x)}$
          \end{enumerate}
    \item Bestimmen Sie die Gleichung der Tangente im Punkt $P$ des Graphen der gegebenen Funktion
          \begin{enumerate}
...
\begin{enumerate}
    \item Betrachten Sie den Graphen der Funktion $f(x)=10^x$. Schätzen Sie den Wert von $f'(-0.4)$, indem Sie die entsprechende Tangente einzeichnen.
          \begin{center}
              \includegraphics[scale=5]{im/Graph10hochx.png}
          \end{center}
    ...
    \item Halten wir fest: Die Ableitung von Exponentialfunktion der Form $y=a^x$ ist ein Vielfaches der Funktion selbst: $y'=c_a\cdot a^x$, wobei der Faktor $c_a$ von der gewählten Basis $a$ abhängig ist. Zum Beispiel ist $c_2 \approx 0.693$ und $c_{10} \approx 2.3026$.
          ...
          Forschen: Finden Sie durch mit Geogebra (oder durch geschicktes Probieren) eine Basis $a$, sodass der entsprechende Faktor $c_a \approx 1$ ist? \\
          \textit{Tipp: Zeichnen Sie die Funktion $y=\dfrac{a^x-1}{x}$; es erscheint ein Schieberegler für $a$; probieren Sie nun verschiedene Werte von $a$ und bestimmen Sie jeweils graphisch den Grenzwert für $x\rightarrow0$. Für welche Basis $a$ ist der Grenzwert ungefähr $1$?}
...
\begin{enumerate}
    \item Bestimmen Sie die Ableitung der folgenden Funktionen:
          ...
          \end{enumerate}
    \item In welchem Kurvenpunkt hat die Funktion $f(x) = e^{2x}$ eine Tangente mit der Steigung $m=8$? Bestimmen Sie die Koordinaten des Punktes.
          ...
          %       \begin{enumerate}
          %           \item Berechnen Sie den Abstand zwischen den Punkten $S$ und $T$.
          %           \item Bestimmen Sie $u$ so, dass die Tangente $t$ durch den Ursprung verläuft.
          %           \item Bestimmen Sie $u$ so, dass das Dreieck $STP$ den Flächeninhalt $A=e$ hat.
          %       \end{enumerate}
\end{enumerate}
\clearpage
\subsubsection*{Lösungen}
...
\subsection{Die Ableitung von Umkehrfunktionen}
\subsubsection{Einführungsaufgabe}
{Spieglein, Spieglein im ersten Quadrant}{}
In dieser Einführungsaufgabe werden Sie den Zusammenhang zwischen den Ableitungen von Funktionen ergründen, die Umkehrfunktionen voneinander sind.
...
\begin{enumerate}
    \item Betrachten wir als erstes die Wurzelfunktionen. Deren Ableitung haben wir schon mit dem Differentialquotienten hergeleitet. In dieser Aufgabe wollen wir die Ableitung der Wurzelfunktion noch einmal auf einem anderen Weg finden, indem wir sie als Umkehrfunktion der Quadratfunktion betrachten. Sei also $f(x)= \sqrt{x}$ und $g(x) = f^{-1}(x)=x^2$ (mit $x\ge0$).
          \begin{center}
              \includegraphics[scale=1.5]{im/spiegleinspieglein.png}
          \end{center}
...
\begin{enumerate}[resume]
    \item Wir betrachten als nächstes die Exponentialfunktion und den Logarithmus zur Basis $e$:
          \begin{center}
              \begin{center}
                  \includegraphics[scale=1.5]{im/spiegleinspiegleinMitLn.png}
              \end{center}
          \end{center}
          \begin{enumerate}
...
\end{enumerate}
\newpage
Die Ergebnisse der Einführungsaufgabe lassen sich wie folgt Zusammenfassen:
\begin{theo}
    {Ableitung von Umkehrfunktionen}{}
    Sei $f: ]a,b[\rightarrow\mathbb{W}$ eine umkehrbare, differenzierbare Funktion und sei $g=f^{-1}$ ihre Umkehrfunktion. Dann gilt:
    $$f'(x) = \frac{1}{g'(f(x))}$$
...
\begin{enumerate}
    \item Bestimmen Sie die Ableitung der folgenden Funktionen:
          ...
          \end{enumerate}
    \item Bestimmen Sie die Ableitung der Funktion $f(x) = \ln(2x)$. Erklären Sie das überraschende Resultat mit dem Logarithmusgesetz $\ln(a\cdot b) = \ln(a) + \ln(b)$.
    \item Bestimmen Sie die Gleichung der Tangente, welche die Kurve $y=e^x\cdot \ln(x)$ in deren Nullstelle berührt.
\end{enumerate}
\clearpage
\subsubsection*{Lösungen}
\begin{enumerate}
...
\end{enumerate}
\clearpage
Einige der Erkenntnisse aus der Einführungsaufgabe – die genauen Rückschlüsse von der Gestalt der Ableitung auf den Graphen der Funktion – werden zu einem späteren Zeitpunkt vertieft werden. Für den Moment halten wir fest, dass eine differenzierbare Funktion mehrmals abgeleitet werden kann:
\begin{deff}
    [Höhere Ableitung]{}
...
\subsubsection*{Übungen}
\begin{enumerate}
    \item Bestimmen Sie die 2. Ableitung der gegebenen Funktion.
          \begin{enumerate}
              \item \( f(x) = 3x^5 + 4x - 2 \)
    ...
    \item
          \begin{enumerate}
              \item Bestimmen Sie die Ableitungen \( f', f'', f^{(3)}, f^{(4)} \) und \( f^{(5)} \) der angegebenen Funktion.
                    \begin{enumerate}
                        \item \( f(x) = x^3 \)
                        \item \( f(x) = x^4 \)
                        \item \( f(x) = x^5 \)
          ...
          % \item Geben Sie eine mögliche Funktionsgleichung \( y = f(x) \) mit der gegebenen Eigenschaft an.
          %       \begin{enumerate}
          %           \item \( y' = x^2 \)
          %           \item \( y'' = x^2 \)
          %           \item \( y''' = x^2 \)
