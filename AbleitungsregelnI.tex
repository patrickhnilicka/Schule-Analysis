...
\end{enumerate}
\clearpage
\subsection{Motivation der Produktregel}

Betrachten wir die Fläche eines Rechtecks. Sowohl die Länge \(l(x)\) als auch die Breite \(b(x)\) hängen von einer Variablen \(x\) ab. Die Fläche berechnet sich als

\[
    A(x)=l(x)\cdot b(x).
\]

Wie verändert sich die Fläche, wenn sich sowohl die Länge als auch die Breite ein wenig vergrössern?
...
\begin{center}
    \begin{tikzpicture}[scale=0.8]
        ...
        \fill[blue!20] (4,0) rectangle (4.6,2);
        \fill[green!20] (0,2) rectangle (4,2.5);

        % Kleine Ecke
        \fill[red!30] (4,2) rectangle (4.6,2.5);

        % Beschriftungen
        \node at (2,1) {$A(x)$};
        \node[right] at (4.6,1) {$l(x)\,\Delta b$};
        \node[above] at (2,2.5) {$b(x)\,\Delta l$};

    \end{tikzpicture}
\end{center}

Die Flächenzunahme setzt sich aus drei Teilen zusammen:

\begin{itemize}
    \item einem vertikalen Streifen,
    \item einem horizontalen Streifen,
    \item der kleinen Ecke oben rechts.
\end{itemize}

Für sehr kleine Änderungen ist die Fläche der Ecke vernachlässigbar. Daher gilt näherungsweise
...
Teilen wir beide Seiten durch \(\Delta x\), erhalten wir

\[
    \frac{\Delta A}{\Delta x}
    \approx
    l(x)\frac{\Delta b}{\Delta x}
    +
    b(x)\frac{\Delta l}{\Delta x}.
\]

Lässt man nun \(\Delta x\) gegen \(0\) gehen, erhält man die \textbf{Produktregel}:
...
Allgemein schreibt man für zwei differenzierbare Funktionen \(f\) und \(g\):
