...
\end{enumerate}
\newpage
\subsection{Tangente an eine Funktion}
\begin{example}
    Sei wieder $f(x)=x^2$. Wie lautet die Gleichung der Tangente, welche den Graphen an der Stelle $x=2$ berührt?

    \kariert{30}

\end{example}
\subsubsection*{Übungen}
Geben Sie jeweils die Gleichung der Tangente der Funktion $f$ an der Stelle $x$ an.
\begin{multicols}{2}
    \begin{enumerate}
        \item $f(x)=x^2,\ x=3$
        \item $f(x)=2x^2-x,\ x=2$
        \item $f(x)=\dfrac{2}{x},\ x=1$
        \item $f(x)=-2x^3,\ x=1$
    \end{enumerate}
\end{multicols}
\subsubsection*{Lösungen}
...
% Nach den Erkenntnissen aus dem Kapitel über Grenzwerte wissen wir, dass diese nicht immer existieren. Somit kann es auch sein, dass für eine gegebene Funktion der Differenzialquotient an einer bestimmten Stelle gar nicht gebildet werden kann.
% \begin{deff}
%     {Differenzierbarkeit}{}
...
\begin{example}{Graph der Ableitungsfunktion}{}
    ...
    \begin{center}
        \includegraphics[scale = 0.7]{im/Ableitungsfunktion.jpg}
    \end{center}
\end{example}
...
\begin{enumerate}
    \item Die Grafiken I bis IV zeigen die ursprünglichen Graphen von vier Funktionen $f$, die Grafiken a) bis d) die Graphen der vier Ableitungsfunktionen $f'$. Welcher $f$-Graph gehört zu welchem $f'$-Graph?
          \begin{center}
              \includegraphics[width = 0.8 \textwidth]{im/zuordnungf-f.png}
          \end{center}
    \item Skizzieren Sie den Graphen der Ableitungsfunktion $f'$ der dargestellten Funktion $f$ in das gleiche Koordinatensystem.
          \begin{center}
              \includegraphics[width = 0.8 \textwidth]{im/graphischAbleiten1.png}
          \end{center}
          \pagebreak
    \item Skizzieren Sie den Graphen der Ableitungsfunktion $f'$ der dargestellten Funktion $f$ in das gleiche Koordinatensystem.
          \begin{center}
              \includegraphics[width = 0.8 \textwidth]{im/GraphischAbleiten 2.png}
          \end{center}
\end{enumerate}
...
\begin{enumerate}
    ...
    \item Ableitungsgraphen
          ...
          \includegraphics[width = 0.8 \textwidth]{im/lsgGraphischAbleiten1.png}

    \item Ableitungsgraphen

          \includegraphics[width = 0.8 \textwidth]{im/LsgGraphischAbleiten2.png}
...
\begin{example}
    \label{bsp:berechnenderAbleitung}
...
\end{example}
\clearpage
Mit der Gleichung der Ableitungsfunktion lässt sich die Geradengleichung der Tangente an einen Funktionsgraphen schneller bestimmen:
...
\begin{enumerate}
    \item
          Bestimmen Sie die Gleichung der Ableitungsfunktion \( f' \) der Funktion \( f  \).

          \begin{enumerate}
              \item \( f(x) = x^2 + 3 \). Vergleichen Sie das Resultat mit Beispiel \ref{bsp:berechnenderAbleitung} und erklären Sie.
              \item \( f(x) = 2x^3 \). Verwenden Sie das Pascalsche Dreieck.
              \item \( f(x) = 3x \). Wieso ergibt das seltsame Resultat Sinn?
              \item \( f(x)= \dfrac{1}{x} \). Achtung Doppelbrüche!
              \item \( f(x) =\sqrt{x} \). Tipp: Erweitern Sie den Differenzenquotienten mit $\sqrt{x+h} + \sqrt{x}$
          \end{enumerate}
    \item Bestimmen Sie die Gleichung der Tangente, welche die Funktion $f$ an der Stelle $x_0$ berührt. Verwenden Sie die Resultate aus Aufgabe 1.
          \begin{enumerate}
...
\begin{enumerate}
    ...
    \item \begin{align*}
              ...
              f'(x)=6x^2-14x                                   \\
              a=f'(3)=12,\ t:y=12x+b                           \\
              P=(3,f(3))=(3,-6) \text{ liegt auf der Tangente} \\
              t(3)=-6,\ 12\cdot 3 + b= -6                      \\
              b=-42                                            \\
              t:y = 12x-42
          \end{align*}
