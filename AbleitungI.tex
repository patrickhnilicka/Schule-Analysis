...
Diese durchschnittliche Änderungsrate beschreibt, wie stark sich eine Grösse \emph{im Durchschnitt} innerhalb des Intervalls $[x, x + \Delta x]$ verändert.
...
Die Sekante nähert sich immer mehr einer Tangente an die Kurve an.

Genau daraus entsteht der Begriff der \textbf{momentanen Änderungsrate}.
\begin{center}
    \begin{tikzpicture}[
        scale=1.0,
...
\begin{deff}[Differentialquotient]{}
    Falls der Grenzwert
    \[
...
\subsection{Übungen}
\begin{enumerate}
    \item Skizzieren Sie den Graphen der Funktion \( f(x) = 2x^2 \). Beantworten Sie dann die folgenden Fragen mit einer Rechnung:
          \begin{enumerate}
    ...
    \item Skizzieren Sie den Graphen der Funktion \( f(x) = x^2 - 5 \).
          \begin{enumerate}
              \item Berechnen Sie \( f'(0) \). Welche Eigenschaft des Graphen wird durch das Resultat verdeutlicht?
              \item Berechnen Sie \( f'(2) \).
              \item Bestimmen Sie \( f'(-2) \). Entweder mit dem Differentialquotient oder indem Sie eine Eigenschaft der Parabel verwenden\dots
          \end{enumerate}

    \item Zeichnem Sie den Graphen der Funktion \( f(x) = \frac{1}{3}x - 1 \).\\
