...
\[
    \frac{f(b)-f(a)}{b-a}
\]
oder etwas anders geschrieben, wenn wir von einem Punkt $x_0$ ausgehen und das Intervall $[x_0, x_0 + \Delta x]$ betrachten:
\begin{align}
    \label{deltaschreibweise}
    \frac{f(x_0+\Delta x)-f(x_0)}{\Delta x}
\end{align}
Diese durchschnittliche Änderungsrate beschreibt, wie stark sich eine Grösse \emph{im Durchschnitt} innerhalb des Intervalls $[x_0, x_0 + \Delta x]$ verändert.
...
Je kleiner das Intervall $[x_0, x_0 + \Delta x]$ wird, desto genauer beschreibt die durchschnittliche Änderungsrate die Änderung der Grösse in der Nähe von $x_0$. Wenn wir das Intervall immer weiter verkleinern, nähern wir uns einer bestimmten Zahl an, die die Änderung der Grösse genau zum Zeitpunkt $x_0$ beschreibt.
...
\begin{example}
    ...
    In der Schreibweise $[x_0,x_0+\Delta x]$ des Intervalls (siehe \eqref{deltaschreibweise}) ist $x_0=1$ und $\Delta x =2$.
\end{example}
% \begin{center}
%     \begin{tikzpicture}[
%         scale=1.0,
...
\begin{enumerate}
    ...
    \item Berechnen Sie die Ableitung der Funktion $f(x) = 2x^3 - 5x^2 + 3$ an der Stelle $x_0 = 2$.
